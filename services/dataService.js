...
export class DataService {
  ...
  static async addOrder(order) {
    try {
      ...
      const newOrderData = { ...order, id: docRef.id, createdAt: { seconds: Math.floor(Date.now() / 1000), nanoseconds: 0 } }; // Mock Firestore Timestamp
      return newOrderData;
    } catch (error) {
      ...
      return { ...order, id: `offline-${Date.now()}` };
    }
  }
